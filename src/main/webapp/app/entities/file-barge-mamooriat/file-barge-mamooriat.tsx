...
export const FileBargeMamooriat = (props: IFileBargeMamooriatProps) => {
  ...
  const getAllEntities = () => {
    props.getEntities(paginationState.activePage - 1, paginationState.itemsPerPage, `${paginationState.sort},${paginationState.order}`, props.location.state?.id || '');
  };
  ...
  const { fileBargeMamooriatList, match, loading, totalItems } = props;
  return (
    <div>
            ...
            <tbody>
              {fileBargeMamooriatList.map((fileBargeMamooriat, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {fileBargeMamooriat.bargeMamooriatId ? (
                      <Link to={`barge-mamooriat/${fileBargeMamooriat.bargeMamooriatId}`}>{fileBargeMamooriat.bargeMamooriatId}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${fileBargeMamooriat.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
      ...
      <div className={fileBargeMamooriatList && fileBargeMamooriatList.length > 0 ? '' : 'd-none'}>
...
const mapStateToProps = ({ fileBargeMamooriat }: IRootState) => ({
  ...
  totalItems: fileBargeMamooriat.totalItems
});
