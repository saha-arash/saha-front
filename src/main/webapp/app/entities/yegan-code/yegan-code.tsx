...
export const YeganCode = (props: IYeganCodeProps) => {
  ...
  const { yeganCodeList, match, loading, totalItems } = props;
  return (
    <div>
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="sahaApp.yeganCode.name">Name</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('code')}>
                  <Translate contentKey="sahaApp.yeganCode.code">Code</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="sahaApp.yeganCode.yegan">Yegan</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="sahaApp.yeganCode.nirooCode">Niroo Code</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {yeganCodeList.map((yeganCode, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{yeganCode.yeganId ? <Link to={`yegan/${yeganCode.yeganId}`}>{yeganCode.yeganId}</Link> : ''}</td>
                  <td>{yeganCode.nirooCodeId ? <Link to={`niroo-code/${yeganCode.nirooCodeId}`}>{yeganCode.nirooCodeId}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${yeganCode.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${yeganCode.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
      ...
      <div className={yeganCodeList && yeganCodeList.length > 0 ? '' : 'd-none'}>
...
const mapStateToProps = ({ yeganCode }: IRootState) => ({
  ...
  totalItems: yeganCode.totalItems
});
